...
//
// Deliberately does NOT skip empty-value fields: PayFast's ITN payload
// includes several optional fields (custom_str1, custom_int1, etc.) as
// empty strings when unused, and PayFast's own signature on their end
// includes them rather than omitting them. Skipping them here was silently
// breaking ITN validation while leaving checkout unaffected (checkout
// fields are all ones we explicitly set, so there was nothing to skip
// there) — which is exactly the "checkout works, ITN doesn't" pattern.
...
async function validateItn(body, passphrase){
  ...
  if(signature !== expected){
    console.error('ITN signature mismatch — received:', signature, '| expected:', expected, '| fields:', JSON.stringify(rest));
