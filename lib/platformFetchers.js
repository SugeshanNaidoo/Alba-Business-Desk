...
async function fetchInstagram(){
  let token;
  // Instagram is its own connection now (api/oauth-instagram.js) — no
  // longer piggybacking on the Facebook Page's linked account, since Meta
  // retired that path in January 2025.
  const conn = await getConnection('instagram').catch(()=>null);
