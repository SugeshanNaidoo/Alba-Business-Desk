...
async function fetchInstagram(){
  ...
  if(!token || !igUserId){
    throw new Error('Instagram is not connected. Use "Continue with Instagram" in Settings, or set IG_ACCESS_TOKEN and IG_BUSINESS_ACCOUNT_ID.');
  }

  const profileRes = await fetch(`https://graph.instagram.com/${igUserId}?fields=followers_count,username&access_token=${token}`);
  const profile = await profileRes.json();
  if(profile.error) throw new Error(profile.error.message || JSON.stringify(profile.error));

  const mediaRes = await fetch(`https://graph.instagram.com/${igUserId}/media?fields=id,caption,timestamp,like_count,comments_count,permalink&limit=25&access_token=${token}`);
