...
async function fetchInstagram(uid){
  let token;
  // Instagram is its own connection now (api/oauth-instagram.js), and
  // per-customer — each signed-in account has its own, not shared
  // workspace-wide.
  const conn = uid ? await getConnection(uid, 'instagram').catch(()=>null) : null;
...
async function fetchFacebook(uid){
  let token, pageId;
  const conn = uid ? await getConnection(uid, 'meta').catch(()=>null) : null;
...
async function fetchTikTok(uid){
  ...
  const conn = uid ? await getConnection(uid, 'tiktok').catch(()=>null) : null;
  ...
  if(usingStoredConnection && tokenData.refresh_token){
    ...
    try{ await setConnection(uid, 'tiktok', { refreshToken: tokenData.refresh_token }); }catch(e){}
  }
