...
// only proves identity and doesn't grant calendar access, and even if it
// did, that access wouldn't outlive the browser tab. This uses a proper
// offline-access OAuth flow (see api/calendar.js) so the backend can check
// availability and manage events at any time, for any visitor, whether the
// business owner is currently signed in or not.
...
async function getFreeBusy(timeMinISO, timeMaxISO){
...
async function listEvents(timeMinISO, timeMaxISO, maxResults){
  const { accessToken, calendarId } = await getAccessToken();
...
async function createEvent({ summary, description, startISO, endISO, attendeeEmail, attendeeName, needsMeet }){
  const { accessToken, calendarId } = await getAccessToken();
  const body = {
    summary,
...
async function updateEvent(eventId, { startISO, endISO, summary, description, attendeeEmails, needsMeet }){
  const { accessToken, calendarId } = await getAccessToken();
  const body = {};
