...
// here takes an orgId: the calendar connection belongs to the ORGANISATION,
// so any member can see and manage its events, and an admin leaving does not
...
async function getAccessToken(orgId){
  const conn = await getConnection(orgId, 'google_calendar');
...
async function listEvents(orgId, timeMinISO, timeMaxISO, maxResults){
  const { accessToken, calendarId } = await getAccessToken(orgId);
...
async function createEvent(orgId, { summary, description, startISO, endISO, attendeeEmail, attendeeName, needsMeet }){
  const { accessToken, calendarId } = await getAccessToken(orgId);
  const body = {
    summary,
...
async function updateEvent(orgId, eventId, { startISO, endISO, summary, description, attendeeEmails, needsMeet }){
  const { accessToken, calendarId } = await getAccessToken(orgId);
  const body = {};
...
module.exports = { getAccessToken, listEvents, createEvent, updateEvent, deleteEvent };
