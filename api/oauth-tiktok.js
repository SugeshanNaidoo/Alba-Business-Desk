...
async function handleStart(req, res){
  ...
  if(!roleAtLeast(startCtx.role, 'admin')){
    return res.status(403).send('Only an owner or admin can connect TikTok for this workspace.');
  }
...
async function handleCallback(req, res){
  ...
  try{
    ...
    await setConnection(ctx.orgId, 'tiktok', {
      refreshToken: tokenData.refresh_token,
      openId: tokenData.open_id,
      displayName,
      connectedAt: Date.now()
    });
    await logEvent('tiktok_connected', { uid: ctx.uid, orgId: ctx.orgId, detail: displayName });
